...
|---|---|---|---|
| 1 | ~~Reconcile `FoodItem.search` with `foodCatalogFilter`, or write down why they differ.~~ | §11 | ✅ **DONE 2026-09-06 (Q41).** `foodCatalogVisibilityFilter(userId)` added to `packages/schemas/fitnessgeek/foodItem.js` — the union of the two old shapes (a legacy no-`user_id`-key row is now visible through both paths, which only widens what an authenticated caller can see). `search` on both sides and `foodRoutes.js`'s `GET /api/foods/:id` now build on it; `ownership.js`'s `foodCatalogFilter` keeps its `requireUser` guard and delegates the filter shape. Both suites' parity tests updated to assert agreement instead of the old disagreement. |
| 2 | **Decide `foodRoutes.js` (`POST /api/foods`) vs `FoodItem.findOrCreate`.** | §11 | ◐ **PARTIALLY DONE 2026-09-06 (Q41).** The *lookup* half is no longer a third ladder — the route now walks the shared `foodItemDedupeFilters` rungs sequentially (barcode → source/source_id → name/brand), so the previously-missing name+brand rung and the "skips source/source_id when a non-matching barcode was also sent" bug are both fixed. The *ownership* half is unchanged and still reported, not decided: this route mints a **user-owned** row (`user_id: userId`) where `findOrCreateFoodItem`'s creation branch always mints a **global** one, and folding that in too would make every custom food typed here instantly visible to every other user — a privacy change, not a refactor. Tests: `apps/fitnessgeek/backend/src/__tests__/routes/foodRoutesCreate.test.js`. |
| 3 | ~~Decide the soft-deleted-barcode collision.~~ | §11 | ✅ **DONE 2026-09-06 (Q40).** `barcode`'s index is now PARTIAL (`partialFilterExpression: { is_deleted: false, barcode: { $type: 'string' } }`) instead of `unique + sparse` — a soft-deleted row's barcode no longer participates in the constraint, so `findOrCreateFoodItem` re-adding a previously-deleted product mints a fresh row instead of `E11000`. Migration: `apps/fitnessgeek/backend/scripts/fixBarcodeUniqueIndex.js` (drops the stale index; each app's own `autoIndex` builds the replacement on its next boot) — **not run against production; Sage runs it.** Both suites' tests updated to prove the fixed behaviour against real Mongo (basegeek side) and the new index shape (fitnessgeek side, hermetic). |
| 4 | **Decide whether the caller-less instance methods live or die** — `NutritionGoals.checkGoalsMet`, `NutritionGoals.getProgress`, `Meal.getNutrition`. Nothing in either app calls any of them | §10, §11 | **Reported 2026-09-06, not deleted** (Q39/Night 2 policy: this is one of the three follow-ups that needs a product call, Chef's). Confirmed still genuinely caller-less by grep, backend and frontend both, on 2026-09-06. Recommendation: delete all three — the byte-equivalence rollback claim they were preserving for has been banked since §11, so keeping them now only costs a discoverability tax. Deleting them touches the parity assertions on both sides (`apps/fitnessgeek/backend/src/__tests__/models/sharedSchemaParity.test.js` and `apps/basegeek/packages/api/src/__tests__/fitnessgeekSchemaParity.test.js`), which is why this stream left the decision to Chef rather than making it unilaterally. |
...
| 6 | ~~`NutritionGoals.checkGoalsMet` and `getProgress` disagree about ceilings.~~ | §10 | ✅ **DONE 2026-09-06 (Q39).** Night 2 policy decision: pick ceiling for both. `computeGoalProgress` in `packages/schemas/fitnessgeek/nutritionGoals.js` now treats sugar/sodium as compliance headroom (100 with nothing eaten, 0 right at the limit, clamped not negative past it) instead of consumed÷goal — hitting the limit exactly no longer reads as "100% progress". Pinned in `apps/fitnessgeek/backend/src/__tests__/models/sharedSchemaParity.test.js`; basegeek's parity test still passes unedited (its one numeric case sits at the 50%-consumed midpoint, which both the old and new formulas agree on). |
| 7 | **`DailySummary.goals_met` treats every macro as a floor**, carbs and fat included — so a keto user under their carb ceiling reads as not meeting the goal | §12 | **Reported 2026-09-06, not changed** — one of the three Night 2 product-decision items (same class as #6 was, one collection over; #6 got an explicit Chef-delegated decision this pass, this one did not). |
| 8 | **Three of `goals_met`'s four flags can never be true.** `UserSettings.nutrition_goal` declares `daily_calorie_target` and no `protein_grams` / `carbs_grams` / `fat_grams` | §12 | **Reported 2026-09-06, not changed** — needs a decision on what a macro goal on the settings document *is* (gram target / ratio / a read of `nutritiongoals`) before any code can move. |
| 9 | **`DailySummary.updateFromLogs` ignores `FoodLog.nutrition`** and reads the current catalog row instead, so editing a food restates every past day that used it | §12 | **Reported 2026-09-06, not changed** — snapshot-immutable vs. catalog-corrections-propagate is a product call, not a refactor. |
| 10 | **`PRE-4` — tighten fitnessgeek's `Meal` list statics.** `getActiveMeals(undefined)` returns every user's meals; basegeek's throw `UNAUTHORIZED`. Every live caller passes a userId, so it is latent | §4 PRE-4, §9, §10 | Unchanged this pass — still explicitly a decision per PRE-4's own text, out of this stream's scope to make unilaterally. |
| 11 | **`LoginStreak`'s doubled `user_id` index and its four timestamp paths.** | §9 | Unchanged this pass — the module header calls this "left alone deliberately" and "a migration, not a refactor" twice over; respected rather than reopened. |
| 12 | **`Weight` and `BloodPressure` use `userId` while the other nine use `user_id`** | §4 Tier 1 note, §8 | Unchanged — explicitly "a data migration wearing a refactor's clothes. Recorded, not fixed." |
| 13 | ~~`FoodLog.nutrition.*` has no `min`~~ | §12 | ✅ **DONE 2026-09-06.** All seven `nutrition.*` leaves in `packages/schemas/fitnessgeek/foodLog.js` now carry `min: 0`, matching `FoodItem.nutrition.*` and `DailySummary.totals.*`. Pinned in `apps/fitnessgeek/backend/src/__tests__/models/sharedSchemaParity.test.js` (rejects a negative value, accepts zero). No existing test asserted the old no-`min` behaviour on either side, so nothing else needed updating. |
| 14 | **`Meal` stamps `updated_at` only on `save()`.** It passes no schema options, so nothing stamps it on a `findOneAndUpdate` / `updateOne`, on either side | §10 | Unchanged — explicitly "shipped behaviour, left alone" in the source doc; not reopened. |
