 ...
 *
 * BuJoGeek's auth payload (from the shared basegeek user service) is not
 * uniform — some users have a `name`, some only a `username`, some only an
 * email — so the sidebar user chip, the top bar account menu and the
 * settings page all funnel through here rather than each guessing
 * differently. Mirrors flockgeek's `utils/userDisplay.js`.
 */

/** Best human-readable name we have, or "Planner" when we have nothing. */
