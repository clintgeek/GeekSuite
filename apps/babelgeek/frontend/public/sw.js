const CACHE_NAME = "babelgeek-cache-v2";
...
self.addEventListener("fetch", (event) => {
  ...
  // Other API: network first, no cache fallback (API data should not be stale)
  if (isApiRequest(url)) {
    event.respondWith(fetch(event.request));
    return;
  }
  ...
  event.respondWith(
    caches.match(event.request).then((cached) => {
      ...
      return fetch(event.request)
        ...
        .catch(() => {
          // Offline: serve fallback for navigation requests
