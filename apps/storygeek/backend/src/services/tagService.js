...
class TagService {
  extractTags(text, storyId, source = 'narrative') {
    ...
    characterMatches.forEach(name => {
      if (name.length > 2 && !this.isCommonWord(name)) {
        tags.push({ tag: 'character', value: name, category: 'character', source, context: `Mentioned in: ${text.substring(0, 100)}...`, relevance: 'high' });
      }
    });
    ...
    locationPatterns.forEach(pattern => {
      ...
      matches.forEach(match => {
        ...
        if (location.length > 2 && !this.isCommonWord(location)) {
          tags.push({ tag: 'location', value: location, category: 'location', source, context: `Mentioned in: ${text.substring(0, 100)}...`, relevance: 'medium' });
        }
      });
    });
    ...
    itemPatterns.forEach(pattern => {
      ...
      matches.forEach(match => {
        ...
        if (item.length > 2 && this.isItemWord(item)) {
          tags.push({ tag: 'item', value: item, category: 'item', source, context: `Mentioned in: ${text.substring(0, 100)}...`, relevance: 'medium' });
        }
      });
    });
    ...
    eventWords.forEach(event => {
      if (text.toLowerCase().includes(event)) {
        tags.push({ tag: 'event', value: event, category: 'event', source, context: `Mentioned in: ${text.substring(0, 100)}...`, relevance: 'high' });
      }
    });
    ...
    conceptWords.forEach(concept => {
      if (text.toLowerCase().includes(concept)) {
        tags.push({ tag: 'concept', value: concept, category: 'concept', source, context: `Mentioned in: ${text.substring(0, 100)}...`, relevance: 'medium' });
      }
    });
  ...
  async saveTags(storyId, tags) {
    try {
      for (const tagData of tags) {
        const existingTag = await StoryTag.findOne({ storyId, tag: tagData.tag, value: tagData.value, category: tagData.category });
        if (existingTag) {
          existingTag.mentionCount++;
          existingTag.lastMentioned = new Date();
          if (tagData.relevance === 'high') existingTag.relevance = 'high';
          await existingTag.save();
        } else {
          const newTag = new StoryTag({ storyId, ...tagData });
          await newTag.save();
        }
      }
    } catch (error) {
  ...
  async queryTags(storyId, query, category = null, limit = 10) {
    try {
      ...
      return await StoryTag.find(searchQuery).sort({ relevance: -1, mentionCount: -1, lastMentioned: -1 }).limit(limit);
    } catch (error) {
  ...
  async getRelevantContext(storyId, currentSituation, limit = 15) {
    try {
      ...
      const sortedTags = uniqueTags.sort((a, b) => {
        if (a.relevance === b.relevance) return b.mentionCount - a.mentionCount;
  ...
  extractKeywords(text) {
    ...
    return words.filter(word => word.length > 3 && !this.isCommonWord(word) && !word.match(/^[0-9]+$/));
  }
