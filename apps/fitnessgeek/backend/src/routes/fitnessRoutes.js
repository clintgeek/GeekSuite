...
router.get('/garmin/status', async (req, res) => {
  try {
    ...
    logger.error({ userId: req.user.id, error: error.message }, 'Failed to get Garmin status');
...
router.get('/garmin/heart-rate/:date?', async (req, res) => {
  try {
    ...
    logger.error({ userId: req.user.id, error: error.message }, 'Failed to get Garmin heart rate');
...
router.get('/garmin/daily/:date?', async (req, res) => {
  try {
    ...
    logger.error({ userId: req.user.id, error: error.message }, 'Failed to get Garmin daily');
    const status = error.message === 'Garmin integration disabled' ? 400 : 500;
    res.status(status).json({ success: false, error: { message: error.message } });
  }
});
...
router.post('/garmin/weight', async (req, res) => {
  try {
    ...
    logger.error({ userId: req.user.id, error: error.message }, 'Failed to push weight to Garmin');
    const status = error.message === 'Garmin integration disabled' ? 400 : 500;
    res.status(status).json({ success: false, error: { message: error.message } });
  }
});
...
router.get('/garmin/sleep/:date?', async (req, res) => {
  try {
    ...
    logger.error({ userId: req.user.id, error: error.message }, 'Failed to get Garmin sleep data');
    const status = error.message === 'Garmin integration disabled' ? 400 : 500;
    res.status(status).json({ success: false, error: { message: error.message } });
  }
});
...
router.get('/garmin/activities', async (req, res) => {
  try {
    ...
    logger.error({ userId: req.user.id, error: error.message }, 'Failed to get Garmin activities');
    const status = error.message === 'Garmin integration disabled' ? 400 : 500;
    res.status(status).json({ success: false, error: { message: error.message } });
  }
});
