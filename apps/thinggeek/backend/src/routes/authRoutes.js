...
// Read per call, not at import: ESM hoists imports above server.js's
// dotenv.config(), so a module-level read would miss .env.local in dev.
const basegeekUrl = () => (process.env.BASEGEEK_URL || 'https://basegeek.clintgeek.com').replace(/\/$/, '');
const APP_NAME = 'thinggeek';
...
function upstreamUnreachable(res) {
  return res.status(502).json({
    message: `Unable to reach baseGeek auth service at ${basegeekUrl()}`,
    code: 'UPSTREAM_UNREACHABLE',
  });
}

/**
 * /api/auth/* — the SSO proxy to basegeek (login/register/refresh/logout
 * are pre-auth by nature; /me is member-gated).
 */
export function createAuthRouter({ memberGate }) {
  ...
  // Member-gated like /api/me: a non-member learns only that they are not one.
  router.get('/me', ...memberGate, meHandler());
