 ...
 *   { typesById: Map, placesById: Map, placeOrder: [id] }
 * from hooks/useThingMeta.js — types and places are household data, so their
 * names come from the cache, not from the facet answer (which carries ids).
 */
...
import { shortPlaceLabel } from './places';
...
export function valueLabel(key, value, context = {}) {
  switch (key) {
    ...
    case 'places': {
      const place = context.placesById?.get(value);
      return place ? shortPlaceLabel(place, context.placesById) : 'Unknown place';
    }
    case 'due':
...
const SECTION_DEFS = [
  ...
  {
    // Tree order, labelled with the place and its parent ("Garage › Shelf 2");
    // a place's count includes everything under it (the server's rule).
    id: 'place',
    title: 'Place',
    kind: 'list',
    key: 'places',
    facet: 'places',
    limit: 10,
    fixed: (context = {}) => context.placeOrder ?? [],
    emptyText: 'No places yet. Add them under Places, then give things a home.',
  },
...
export const DEFAULT_OPEN = { type: true, place: true, due: true, tags: true, missing: true, files: false, year: false, value: false };
