 ...
 * facets), each with its live count.
 *
...
import { visibleOptions } from '../facets/options';
...
export function OptionRow({ option, type = 'checkbox', name, onChange }) {
  ...
  const accessibleName = [label, hint, count == null ? null : countNoun(count, noun)].filter(Boolean).join(', ');
