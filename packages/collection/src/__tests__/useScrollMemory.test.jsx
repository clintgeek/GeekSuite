/**
 * Scroll memory per view (moved from GameGeek with the hook; its library
 * passes storageKey 'gamegeek.libraryScroll').
 */
import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { renderHook } from '@testing-library/react';
import { useScrollMemory } from '../hooks/useScrollMemory';

const KEY = 'gamegeek.libraryScroll';
const useLibraryScrollMemory = (root, key, opts) => useScrollMemory(root, key, { ...opts, storageKey: KEY });
...
const remembered = (map) => window.sessionStorage.setItem(KEY, JSON.stringify(map));
...
describe('useScrollMemory', () => {
  ...
  it('switching views mid-page resets, then remembers the new one as you scroll', async () => {
    ...
    expect(JSON.parse(window.sessionStorage.getItem(KEY))).toEqual({ '': 1500 });
